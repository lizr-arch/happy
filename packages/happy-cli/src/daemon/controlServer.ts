    ...
    typed.post('/spawn-session', {
      schema: {
        body: z.object({
          ...
          agent: z.enum(['claude', 'codex', 'gemini', 'openclaw', 'reasonix']).optional(),
      ...
      const { directory, sessionId, agent, environmentVariables, model } = request.body;

      logger.debug(`[CONTROL SERVER] Spawn session request: dir=${directory}, sessionId=${sessionId || 'new'}, agent=${agent || 'default'}, model=${model || 'none'}`);
      const result = await spawnSession({ directory, sessionId, agent, environmentVariables, model });
