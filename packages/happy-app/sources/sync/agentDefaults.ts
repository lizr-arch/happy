...
export const agentKeys = ['claude', 'codex', 'gemini', 'openclaw', 'reasonix'] as const;
...
export const AgentDefaultOverridesSchema = z.object({
    ...
    reasonix: AgentDefaultOverrideSchema.optional(),
}).passthrough().default({});
...
export function normalizeAgentKey(flavor: string | null | undefined): AgentKey {
    if (flavor === 'codex' || flavor === 'gemini' || flavor === 'openclaw' || flavor === 'reasonix') {
        return flavor;
    }
    return 'claude';
}
